...
import tbInspFooterActionCreateNoteNG from 'c/tbInspFooterActionCreateNoteNG';
import tbInspFooterActionCreateTaskNG from 'c/tbInspFooterActionCreateTaskNG';


export default class TbInspQuestionFooterActionsNG extends NavigationMixin(LightningElement) {


    @api question;
    @api inspectionRec;
    @api templateName;
    @api ispreviewshown = false;
    ...
    get isFileTypeQuestion() {
        return this.question.fieldType=='File' || this.question.fieldType=='Signature';
    }
    
    
    async handleAddNote(event){
        //this.showNoteCreationPopup = true;
        const result = await tbInspFooterActionCreateNoteNG.open({
            // `label` is not included here in this example.
            // it is set on lightning-modal-header instead
            inspectionRec: this.inspectionRec,
            templateName : this.templateName,
            question: this.question,
            noteId: this.question.noteId,
            ispreviewshown:this.ispreviewshown,
            size: 'medium',
            description: '',
            content: '',
            onnoterecordcreate: (e) => {
                e.stopPropagation();
                this.maintainVisitNotesRecords(e);
            },
            onclose: (e) => {
                e.stopPropagation();
                this.closeNotePopupModel(e);
            }
        });
    }
    ...
    async handleAddTask(event) {
        ...
        //this.showNoteCreationPopup = true;
        const result = await tbInspFooterActionCreateTaskNG.open({
            // `label` is not included here in this example.
            // it is set on lightning-modal-header instead
            inspectionRec: this.inspectionRec,
            templateName : this.templateName,
            ...
            description: '',
            content: '',
            ontaskrecordcreate: (e) => {
                e.stopPropagation();
                this.maintainTaskRecords(e);
            },
            onclose: (e) => {
                e.stopPropagation();
                this.closeTaskPopupModel(e);
            }
        });
    }
