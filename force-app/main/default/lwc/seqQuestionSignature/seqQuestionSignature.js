...
export default class SeqQuestionSignature extends LightningElement {
    ...
    renderedCallback() {
        ...
        if(this.filesList!= undefined && this.filesList != null 
            && this.filesList.hasOwnProperty(this.question.surveyQuestionId)){
            var existingCtx = this.canvasElement.getContext("2d");
            var image = new Image();
            image.onload = function() {
                existingCtx.drawImage(image, 0, 0);
            };                    
            image.src = this.filesList[this.question.surveyQuestionId];
        }
     }
