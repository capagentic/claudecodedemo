...
import fetchLookupDataForFlowObj from '@salesforce/apex/CustomLookupLwcController.fetchLookupDataForFlowObj';
...
export default class CustomLookupForScreenFlow extends LightningElement {

    // public properties with initial default values 
    @api label = 'custom lookup label';
    @api placeholder = 'search...';
    @api iconName = 'standard:flow';
    @api sObjectApiName = 'FlowDefinitionView';
    @api defaultRecordId = '';
    @api parentUid;
    @api searchKey ;
   ...
   // searchKey = ''; // to store input field value    
    ...
    @wire(fetchLookupDataForFlowObj, { searchKey: '$searchKey', sObjectApiName: '$sObjectApiName' })
    searchResult(value) {
    ...
    handelSelectedRecord(event) {
        ...
        this.selectedRecord = this.lstResult.find(data => data.ApiName === objId); // find selected record from list 
    ...
    lookupUpdatehandler(value) {
        const oEvent = new CustomEvent('lookupupdate',
            {
                'detail': { selectedRecord: value, parentUid: this.parentUid }
            }
        );
    ...
    handleonfocus(event) {
        //created custome event same as standard event so that it can help to retrieve value in same way
        const oEvent = new CustomEvent('focus',
        {
            'target' :{'dataset':{'uid': this.parentUid} }
        });
        this.dispatchEvent(oEvent);
    }
    ...
    handleofffocus(event) {
        //created custome event same as standard event so that it can help to retrieve value in same way
        const oEvent = new CustomEvent('blur',
        {
            'target' :{'dataset':{'uid': this.parentUid} }
        });
        this.dispatchEvent(oEvent);
