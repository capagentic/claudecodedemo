...
export default class TbInspQuestionGeoLocationNG extends LightningElement {
    ...
    renderedCallback() {
        if (this.isRendered == false && this.displayMap == false) {
            this.address = this.question.answer;
        }

        if (!this.isRendered && this.displayMap) {
            this.getCurrentBrowserLocation();
            const style = document.createElement('style');
            style.innerText = `c-tb-insp-question-geo-location-n-g .slds-map {
            ...
            .then(result => {
                ...
                if (resObj.statusCode == '2000') {
                    ...
                    if (this.addresses.length > 0) {
                        ...
                        this.address = this.addresses[0] + ' (' + latitude + ',' + longitude + ')';
                        const valueChangeEvent = new CustomEvent('valuechanged', {
                            detail: { index: this.question.index, answer: this.address, question: this.question }
                        });
                        this.dispatchEvent(valueChangeEvent);
                        ...
                        this.address = '(' + latitudeLongitude + ')';
                        const valueChangeEvent = new CustomEvent('valuechanged', {
